...
export class VideoService {
    ...
    async GetVideos(channelId: string, forceRefresh: boolean): Promise<Video[]> {
        ...
        const youtubeVideos = await this.youtubeAPI.GetYoutubeChannelVideos(channelId, { maxResults: this.VIDEOS_NUMBER_LIMIT });
        if (!youtubeVideos?.items?.length) return [];

        const dbVideos: Video[] = youtubeVideos.items.map(video => ({
            id: video.id?.videoId || '',
    ...
    async InsertVideoList(videoList: Video[]): Promise<number> {
        ...
        const values = videoList.map(video =>
            columns.map(column => video[column])
        );
