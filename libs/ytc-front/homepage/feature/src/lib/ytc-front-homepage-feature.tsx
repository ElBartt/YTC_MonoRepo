import { Button, InputTextMultiLabel } from '@ytc/shared/ui-components-react';
...
export function YtcFrontHomepageFeature() {
    ...
    const handleClick = (): void => {
        getUser(
            import.meta.env.DEV && inputValue === '' ? import.meta.env.VITE_APP_API_KEY_MASTU : inputValue,
        ).subscribe(user => {
            if (user) {
                navigate('channels', {
                    state: {
                        user,
                        apiKey:
                            import.meta.env.DEV && inputValue === ''
                                ? import.meta.env.VITE_APP_API_KEY_MASTU
                                : inputValue,
                    },
                });
            } else {
