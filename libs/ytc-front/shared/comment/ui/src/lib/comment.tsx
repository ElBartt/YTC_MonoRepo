...
export const getCommentType = (com: CommentType) => {
    return com.collaboration ? 'collaboration' : com.feedback ? 'feedback' : com.gpt ? 'gpt' : com.question ? 'question' : 'unwanted';
}
...
export function Comment({comment}: CommentProps) {
    ...
    return (
        <div className="card-body px-0" hidden={false}>
            <h2 className="card-title"
                dangerouslySetInnerHTML={{__html: comment.commenter}}></h2>
            ...
            <div className="flex items-center w-1/4">
                ...
                <p className="italic text-xs">{getCommentType(comment)}</p>
            </div>
        </div>
    );
}
